...
> IN DRAFT: To do:    
>    * Adopt Garrett's `get()` and `check()` notation for locator functions. That will be better than the hodge-podge of `fcall()`, `assigns_to()`, ...     
...
* `assigns_to("var")` looks for a statement which involves assignment to the variable named in the argument (`"var"` in this example). It can also take a regex. 
...
* `fcall("cos(x + whatever)")` looks for a statement containing a function call that matches the one provided as an argument. 
...
USER_CODE %>% 
  ...
  test_3 %>% final_ %>% check_value(match_number(cos(pi/2)))
```
