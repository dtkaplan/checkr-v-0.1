...
match_values(USER_CODE, SOLN_CODE,
  ...
  show_results()
```
The `match_values()` function from `checkr` compares the values produced by the student's code and the values produced by code provided by the test writer in a `-solution` chunk. `USER_CODE` and `SOLN_CODE` convey the contents of the submission and solution chunks, respectively. 

The next argument to `match_values()` uses a function `final_()`, which looks for the *last* value produced by the student and by the solution code. In this example, there's only one value, but in general there may be multiple values produced by submitted code. Whatever that last value be, and regardless of whether the student's or solution code assigned a name to the value, the argument name will be assigned to that value for future reference.

The final argument to `match_values()` refers to the name `ref` created in the previous argument. The `same_num()` function compares the `ref` value in the student and the solution code. 

Later, we'll show examples using `match_values()` that can deal with more complicated structure in the student and solution code. For instance, you can have as many reference-named values and comparison tests as you like, but that's hardly needed in this simple example.
...
Often, perhaps most of the time, the question author will provide an example solution. The `match_values()` function lets you compare the values generated by the solution to the values generated by the user's code.
...
match_values(USER_CODE, SOLN_CODE,
  ...
  show_results
```
The first two arguments to `match_values()` are always the user code and the solution code in that order. The following arguments to `match_values()` are either named or unnamed. 
...
In operation, `match_values()` extracts out the values from the user code and the solution code for the command that passes the locator statement. The comparison tests are then run to compare the user values and the solution values.
