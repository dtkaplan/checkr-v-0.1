...
The documentation for the `tutor` package has instructions and examples for creating exercises. In a nutshell, you create an ordinary Rmd file with code chunks labelled to identify the exercise and the various components of the exercise:
...
The third line in the chunk tells `tutor` to use the `checkr` system. Use it exactly as shown below. 
...
The file created by `new_exercise()` is contains the properly named `tutor` chunks for one exercise. I develop and test the exercise using just that file. But for deploying the exercise, I create another `tutor` file that includes single-exercise files as child documents. That file looks like this:
...
In deploying `tutor` exercises in this one-per-file manner, make sure that both the parent document and all the child documents are installed on the server. 

Note that `tutor` has a template for interactive tutorial documents that contains multiple exercises. You may well prefer to work with that format
