...
test_that("match_the_arguments works with named args", {
  ...
  expect_true(checkr:::match_the_arguments(actual, desired))
  actual <- as.list(parse(text = "aes(x = hp, y = mpg)")[[1]])
  ...
  desired <- as.list(parse(text = "aes(x = mpg, y = NULL)")[[1]])
  expect_false(checkr:::match_the_arguments(actual, desired)) # should be FALSE
})
...
test_that("Looking for mistakes works", {
  test_1 <- fcall("lm(hp ~ mpg, data = whatever)", mistake = TRUE, "Response should be 'mpg'.")
  one <- example_1 %>% test_1
  expect_true(one$passed) # the mistake wasn't in the submitted code
  test_2 <- fcall("lm(mpg ~ hp, data = whatever)", mistake = TRUE,
