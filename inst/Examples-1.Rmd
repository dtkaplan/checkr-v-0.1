...
test_0 <- find_call("c(1,1,1,2,2)", "Use `rep()` for this exercise, not `c()`", mistake = TRUE)
...
test_4 <- find_statement(regex = "each *=", message = "use the `each = ` argument to `rep()`")
...
test_3 <- find_statement(regex = "each *=", message = "use the `each = ` argument to `rep()`")
test_4 <- check_argument("rep(whatever, each = grab_this)", match_number(4))
test_5 <- find_call("rep(eval(letters[1:3]), each = 4)", message = 
