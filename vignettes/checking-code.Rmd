...
> IN DRAFT: To do:    
>    * Adopt Garrett's `get()` and `check()` notation for locator functions. That will be better than the hodge-podge of `find_call()`, , ...   using `find_` rather than get  
>    * Add examples for `magrittr` pipelines.
>    * add `test=` to all `find_` functions



In tutorial systems such as [DataCamp.com](http://DataCamp.com) or the `tutor` package in R, problems are posed that call for the student to write an answer in R code. This package provides a system for checking whether that R code has done what's required and, if not, where the problem might fall. A working example, using `tutor` is available via ... [right now, it's in `inst/checkr-test-cases.Rmd`. Just open that in an editor and "Run Document." Try making small changes in the answers and see what the feedback messages say.]
...
The tests here are called "locator tests." All three of `test_1`, `test_2` and `test_3` here are constructed by the `find_call()` function in `checkr`. The `find_call()` function takes as it's first argument a pattern to look for in the submitted code. The second argument is a message to give if the pattern is *not* found; this is called the "failure message." 
...
* `in_statements()` looks for a character string or regex match to the body of a statement.
* `in_values()` looks for a match to the value produced by a statement
...
* `has_names(sqrt(3 * x + y))` will look for a statement that contains function and object names matching those in the argument. Note that the argument is unquoted.
* `has_constants(3, "skippidee")` looks for a statement with numbers and character strings matching those given in the arguments. You can include as many constants as you like.
* `has_formula()` looks for a statement containing a formula.
